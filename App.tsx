...
const App: React.FC = () => {
  ...
  const handleGenerateOutfits = useCallback(async () => {
    if (!fullBodyImage || !clothing || !clothingImage) return;
    setIsLoading(true);
    ...
    try {
      ...
      const imagePromises = angles.map((angle) =>
        generateTryOnImage2(fullBodyImage, clothing, angle.prompt)
      );
      // const imagePromises = angles.map((angle) =>
      ...
      console.error(err);
      setError(
        "コーディネート画像の生成に失敗しました。もう一度お試しください。"
      );
      setCurrentStep(AppStep.UPLOAD_CLOTHES);
      setClothingImage(null);
    } finally {
        ...
        return (
          <div className="text-center">
            ...
            <p className="text-gray-500 mb-6">
              あなたの顔がはっきりと写っている写真をアップロードしてください。
            </p>
      ...
      case AppStep.GENERATE_BODY:
        return (
          <div className="text-center">
            ...
            <p className="text-gray-500 mb-6">
              アップロードされた写真をもとに、AIがあなたの全身画像を生成します。
            </p>
              ...
              <div className="text-center hidden md:block">
                ...
                {fullBodyImage && (
                  <img
                    src={fullBodyImage}
                    alt="Full Body"
                    className="rounded-lg shadow-md w-48 h-72 object-cover bg-gray-200"
                  />
                )}
              </div>
            ...
            <div className="flex flex-wrap gap-4 items-center justify-center mb-6">
              {fullBodyImage && (
                <img
                  src={fullBodyImage}
                  alt="Full Body"
                  className="rounded-lg shadow-md w-40 h-64 object-cover"
                />
              )}
              ...
              {clothingImage && (
                <img
                  src={clothingImage}
                  alt="Clothing"
                  className="rounded-lg shadow-md w-40 h-64 object-contain bg-white p-2"
                />
              )}
              <div>
                <p>商品名</p>
        ...
        return (
          <div className="text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button
                onClick={() => window.open(clothing.itemUrl, "_blank")}
